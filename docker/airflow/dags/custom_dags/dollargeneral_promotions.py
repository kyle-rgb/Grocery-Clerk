...
with DAG(
    ...
    def setup_browser_and_extract(ds=None, **kwargs):
        ...
        client = docker.from_env()
        scrapingContainer = list(filter(lambda x: x.name=="docker-scraper-1"), client.containers.list())[0]
        scrapingContainer.exec_run("node ./src/index.js --dollar-general promotions", user="pptruser", workdir="/app")
        client.close()
        return 0
    ...
    def transform_publix_item_data(ds=None, **kwargs):
        ...
        client = docker.from_env()
        scrapingContainer = list(filter(lambda x: x.name=="docker-scraper-1"), client.containers.list())[0]
        scrapingContainer.exec_run("node ./src/transform.js transform --dollar-general promotions", user="pptruser", workdir="/app")
        client.close() 
        return 0
    # [END kroger_operator_setup_node]
