...
with DAG(
    dag_id="food_depot_scrape_items",
    ...
    def setup_browser_and_extract(ds=None, **kwargs):
        ...
        scrapingContainer.exec_run("node ./src/index.js --food-depot items", user="pptruser", workdir="/app")
        client.close()
        return 0
    ...
    # [START kroger_operator_setup_node]
    @task(task_id="transform_items_data_food_depot")
    def transform_food_depot_item_data(ds=None, **kwargs):
        ...
        client = docker.from_env()
        scrapingContainer = list(filter(lambda x: x.name=="docker-scraper-1"), client.containers.list())[0]
        scrapingContainer.exec_run("node ./src/transform.js transform --food-depot items", user="pptruser", workdir="/app")
        client.close() 
        return 0
    # [END kroger_operator_setup_node]
