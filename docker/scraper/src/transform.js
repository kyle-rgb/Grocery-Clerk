...
const tryZip = () => {
    ...
    let ds = (new Date()).toLocaleDateString().replace("/", "_")
    var cmd = spawn("tar", ["-cvzf", "-", "../../../data/collections", "|", "gpg", "--output", `${ds}.tar.gz.gpg`, '--encrypt', '--recipient', process.env.EMAIL])
    var cmd = spawn("tar", ["-cvzf", "../attempt.tar.gz", "../attempt"])
    cmd.stderr.on("data", (data)=> console.log(data.toString()))
    cmd.on("close", (code)=> {console.log('process exited with code ', code)})
}
