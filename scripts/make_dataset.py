...
# simulateUser(link); updateGasoline(data); getKrogerTrips(); getFamilyDollarItems(); getScrollingData(chain: str); getPublixCouponData(deals=673); getDGItems(); deconstructDollars(file); backupDatabase(); deconstructKrogerFiles(filename);normalizeStoreData();
...
#   updateGasoline(data), << deconstructKrogerFiles()
...
#  deconstructKrogerFiles(filename), << [pytz, re, json, datetime, ^retrieveData[os, MONGO_CONN_URL, pymongo], ^updateGasoline[], ^insertData[os, MONGO_CONN_URL, pymongo], ^insertFilteredData[os, MONGO_CONN_URL, time, pymongo]]
...
#   << [os, pytz, json, ^insertFilteredData, datetime, os, ^deconstructKrogerFiles, ^deconstructDollars, ^normalizeStoreData, ^backupDB, ^createDBSummaries]
# ]
    ...
    if wantedPaths: 
        for head, subfolders, files in walkResults:         
            if head.split('\\')[-1] in wantedPaths:
                ...
                for file in files:
                    deconstructKrogerFiles(head+"\\"+file)
